...
import { Loader2, Save, Upload, X, Cloud, Laptop, Monitor, Keyboard, ChevronRight, ChevronLeft, Check, Package, ImageIcon, Settings } from "lucide-react";
...
export function ProductEditDialog({ product, open, onOpenChange, onSave, isLoading }: ProductEditDialogProps) {
  ...
  const steps: Step[] = isAccessory ? ['basic', 'images'] : ['basic', 'specs', 'images'];

  const stepLabels: Record<Step, { label: string; icon: React.ElementType }> = {
    basic: { label: 'Basic', icon: Package },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-[500px] h-[85vh] sm:h-auto sm:max-h-[85vh] glass-card border-border/50 p-0 flex flex-col overflow-hidden">
        ...
        <DialogHeader className="px-4 py-3 border-b border-border/30 shrink-0">
          <DialogTitle className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <span className="text-base font-semibold">Edit {catInfo?.label || 'Product'}</span>
            ...
            {steps.map((step, index) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 flex flex-col items-center gap-1 p-2 rounded-lg transition-all cursor-pointer ${
                    isActive 
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          <form id="edit-form" onSubmit={handleSubmit} className="p-4 space-y-4">
