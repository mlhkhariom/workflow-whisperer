...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case "add-laptop": {
        // Get next row_number
        const maxRowLaptop = await client.queryObject<{ max: number }>(
          `SELECT COALESCE(MAX(row_number), 0) as max FROM laptops`,
        );
      ...
      case "add-desktop": {
        // Get next row_number
        const maxRowDesktop = await client.queryObject<{ max: number }>(
          `SELECT COALESCE(MAX(row_number), 0) as max FROM desktops`,
        );
      ...
      case "add-accessory": {
        // Get next row_number
        const maxRowAccessory = await client.queryObject<{ max: number }>(
          `SELECT COALESCE(MAX(row_number), 0) as max FROM accessories`,
        );
      ...
      case "get-chats":
        result = await client.queryObject(`
          SELECT id, contact_uid, role, content, created_at, mobile_number, image_url, name
          FROM chat_messages 
